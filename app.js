...
async function generatePDF() {
  ...
  const addText = (label, value, indent = 0) => {
    ...
    doc.text(String(value), margin + 60 + indent, y);
    y += 6;
  };
  ...
  // Header
  addTitle('PLC MEMORY CAPACITY REQUIREMENTS', 18);
  y += 2;
  addTitle('REPORT', 14);
  y += 10;
  ...
  doc.text('MINIMUM REQUIRED PLC CAPACITY', pageWidth / 2, y + 10, { align: 'center' });
  
  doc.setFontSize(20);
  doc.text(`${formatMb(results.totalMb)} Mb`, pageWidth / 2, y + 22, { align: 'center' });
  
  doc.setFontSize(9);
  doc.setFont('helvetica', 'normal');
  doc.text(`(${formatNumber(results.total)} bytes)`, pageWidth / 2, y + 28, { align: 'center' });
  ...
  doc.text('Value (bytes)', margin + 90, y + 3);
  doc.text('Percentage', margin + 135, y + 3);
  y += 10;
  ...
  categories.forEach((cat, i) => {
    ...
    doc.text(formatNumber(cat.value), margin + 90, y);
    doc.text(`${(cat.value / results.total * 100).toFixed(1)}%`, margin + 135, y);
    y += 6;
  });
  ...
  doc.text(formatNumber(results.total), margin + 90, y + 1);
  doc.text('100%', margin + 135, y + 1);
  y += 15;
  ...
  doc.text(`Page 1 of 1  |  Generated: ${new Date().toLocaleString('en-GB')}  |  PLC Capacity Calculator v${APP_VERSION}`, pageWidth / 2, pageHeight - 10, { align: 'center' });
